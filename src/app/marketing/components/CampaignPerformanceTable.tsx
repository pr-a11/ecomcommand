...
function RoasValue({ roas }: { roas: number }) {
  if (roas >= 3) return <span className="text-sm font-bold tabular-nums text-emerald-600">{roas.toFixed(2)}x</span>;
  if (roas >= 1.5) return <span className="text-sm font-bold tabular-nums text-amber-600">{roas.toFixed(2)}x</span>;
  return <span className="text-sm font-bold tabular-nums text-red-600">{roas.toFixed(2)}x</span>;
}

function PlatformBadge({ platform }: { platform: string }) {
  if (platform === 'meta') {
    return <span className="text-xs font-bold px-1.5 py-0.5 rounded bg-indigo-50 text-indigo-600 flex-shrink-0">M</span>;
  }
  return <span className="text-xs font-bold px-1.5 py-0.5 rounded bg-sky-50 text-sky-600 flex-shrink-0">G</span>;
}

function SortIcon({ active, dir }: { active: boolean; dir: SortDir }) {
  if (!active) return <ChevronDown size={9} className="text-gray-300" />;
  return dir === 'desc' ? <ChevronDown size={9} className="text-gray-700" /> : <ChevronUp size={9} className="text-gray-700" />;
}
  ...
  return (
    <div className="bs-chart-card">
      ...
      <p className="bs-chart-subtitle mb-3">
        All channels · {campaignData.length} campaigns · Spend {formatINR(totalSpend)} · Attributed Sales {formatINR(totalAttrSales)} · Attributed ROAS {blendedRoas.toFixed(2)}x · reported by Meta Ads + Google Ads
      </p>
          ...
          <thead>
            <tr className="border-b border-gray-100">
              <th className="text-left pb-2.5 pr-3 min-w-[180px]">
                <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Campaign</span>
              </th>
              <th
                ...
                >
                  <td className="py-2.5 pr-3 min-w-[180px]">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-gray-800 truncate max-w-[155px]">{row.name}</span>
                    </div>
                  </td>
        ...
        <table className="w-full min-w-[520px]">
          <tfoot>
            <tr className="border-t border-gray-200 bg-gray-50/50">
              ...
              <td className="py-2.5 pr-3 text-right tabular-nums text-sm font-semibold text-gray-700">{formatINR(totalSpend)}</td>
              <td className="py-2.5 pr-3 text-right tabular-nums text-sm font-bold text-gray-800">{formatINR(totalAttrSales)}</td>
              <td className="py-2.5 pr-3 text-right tabular-nums text-sm font-semibold text-gray-700">{totalOrders}</td>
              <td className="py-2.5 pr-3 text-right">
