      ...
      {payload.map((p: any) => (
        <div key={`ag-${p.dataKey}`} className="flex justify-between gap-4 mb-1">
          ...
          <span className="text-xs font-600 text-foreground">
            {p.dataKey === 'visitors' ? p.value.toLocaleString('en-IN')
              : formatINR(p.value)}
          </span>
        </div>
      ))}
    ...
    <div className="chart-card">
      ...
      <ResponsiveContainer width="100%" height={220}>
        <ComposedChart data={ageGenderData}>
          ...
          <Bar yAxisId="left" dataKey="spend" name="Ad Spend" fill="var(--channel-meta)" opacity={0.7} barSize={18} isAnimationActive animationDuration={800} />
          <Bar yAxisId="left" dataKey="sales" name="Attr. Sales" fill="var(--primary)" opacity={0.85} barSize={18} isAnimationActive animationDuration={900} />
          <Line yAxisId="right" type="monotone" dataKey="visitors" name="visitors" stroke="var(--channel-amazon)" strokeWidth={2} dot={{ r: 3, fill: 'var(--channel-amazon)' }} isAnimationActive animationDuration={1000} />
          <Legend
      ...
      <div className="mt-4 pt-4 border-t border-border">
        <p className="text-xs font-600 text-muted-foreground uppercase tracking-wider mb-3">Gender Split (Meta Ads buyers)</p>
        <div className="space-y-2">
          {[
            { label: 'Female', value: ageGenderData.reduce((s, d) => s + d.female, 0), color: 'var(--channel-myntra)' },
            { label: 'Male', value: ageGenderData.reduce((s, d) => s + d.male, 0), color: 'var(--channel-flipkart)' },
          ].map((g) => {
