...
function StatCard({
  ...
  return (
    <div
      className={`bg-white rounded-xl border shadow-sm p-4 ${isAlert ? 'border-red-200 bg-red-50' : 'border-gray-100'}`}
    >
      <p className={`text-xs font-medium mb-1 ${isAlert ? 'text-red-500' : 'text-gray-500'}`}>
        {label}
      </p>
      <p className={`text-3xl font-bold mb-1 ${isAlert ? 'text-red-600' : 'text-gray-900'}`}>
        {value}
      </p>
      {sub && <p className="text-xs text-gray-400">{sub}</p>}
      {change !== undefined && (
        <span
          className={`inline-flex items-center gap-0.5 text-xs font-semibold px-1.5 py-0.5 rounded-full mt-1 ${change >= 0 ? 'bg-gray-100 text-gray-800' : 'bg-red-50 text-red-600'}`}
        >
          {change >= 0 ? '↑' : '↓'} {Math.abs(change)}
          {typeof change === 'number' && change < 5 ? '' : '%'}
        </span>
      )}
    </div>
  );
      ...
      <div className="space-y-6">
        ...
        {operationsKpis && (
          <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
            <StatCard
              label={operationsKpis.dispatched.label}
              value={operationsKpis.dispatched.value}
              sub={operationsKpis.dispatched.sub}
            />
            <StatCard
              label={operationsKpis.pending.label}
              value={operationsKpis.pending.value}
              sub={operationsKpis.pending.sub}
              isAlert
            />
            <StatCard
              label={operationsKpis.inTransit.label}
              value={operationsKpis.inTransit.value}
              sub={operationsKpis.inTransit.sub}
            />
            <StatCard
              label={operationsKpis.deliveredToday.label}
              value={operationsKpis.deliveredToday.value}
              change={operationsKpis.deliveredToday.change}
            />
            <StatCard
              label={operationsKpis.returnRequests.label}
              value={operationsKpis.returnRequests.value}
              change={operationsKpis.returnRequests.change}
            />
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
              <p className="text-xs text-gray-500 font-medium mb-1">
                {operationsKpis.avgDeliveryDays.label}
              </p>
              <p className="text-3xl font-bold text-gray-900 mb-1">
                {operationsKpis.avgDeliveryDays.value}
              </p>
                ...
                <thead>
                  <tr className="bg-gray-50">
                    {[
                      ...
                      <th
                        key={h}
                        className="text-left text-xs font-semibold text-gray-500 px-4 py-2.5"
                      >
                        {h}
                      </th>
                    ))}
                  ...
                  {courierData?.map((row: any) => (
                    <tr key={row.courier} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`text-sm font-semibold ${row.deliveredPct >= 95 ? 'text-gray-800' : row.deliveredPct >= 90 ? 'text-amber-600' : 'text-red-600'}`}
                        >
                          {row.deliveredPct}%
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`text-sm font-semibold ${row.ndrRate <= 3 ? 'text-gray-800' : row.ndrRate <= 5 ? 'text-amber-600' : 'text-red-600'}`}
                        >
                          {row.ndrRate}%
                        </span>
                      </td>
            ...
            <div className="space-y-2.5">
              {deliveryByStateData.map((row) => (
                <div key={row.state} className="flex items-center gap-3">
                  ...
                  <span
                    className={`text-xs font-semibold w-12 text-right ${row.onTime >= 95 ? 'text-gray-800' : row.onTime >= 90 ? 'text-amber-600' : 'text-red-600'}`}
                  >
                    {row.onTime}%
                  </span>
                </div>
              ...
              <tbody className="divide-y divide-gray-50">
                {ndrData?.map((row: any) => (
                  <tr key={row.orderId} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`text-xs font-semibold px-2 py-0.5 rounded-full ${row.status === 'RTO' ? 'bg-red-100 text-red-700' : row.status === 'Reattempt' ? 'bg-amber-100 text-amber-700' : 'bg-blue-100 text-blue-700'}`}
                      >
                        {row.status}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <button className="text-xs text-teal-600 font-semibold hover:text-teal-800 transition-colors">
                        Resolve
                      </button>
                    </td>
                  </tr>
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          <div className="divide-y divide-gray-50">
            {inventoryAlerts?.map((item: any) => (
              ...
              >
                <span
                  className={`text-xs font-bold px-2 py-0.5 rounded-full uppercase ${urgencyColor(item.urgency)}`}
                >
                  {item.urgency}
                </span>
