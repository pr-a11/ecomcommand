...
const reportTemplates = [
  { id: 1, icon: '📊', title: 'Monthly Performance Report', desc: 'Sales, orders, revenue & channel breakdown', lastGenerated: '1 Aug 2024', category: 'Sales' },
  { id: 2, icon: '🎯', title: 'Channel Attribution Report', desc: 'Multi-touch attribution across all channels', lastGenerated: '28 Jul 2024', category: 'Marketing' },
  { id: 3, icon: '💰', title: 'Campaign ROI Report', desc: 'Ad spend, ROAS & campaign performance', lastGenerated: '25 Jul 2024', category: 'Marketing' },
  { id: 4, icon: '📦', title: 'Inventory Health Report', desc: 'Stock levels, turnover & reorder analysis', lastGenerated: '20 Jul 2024', category: 'Operations' },
  { id: 5, icon: '👥', title: 'Customer Cohort Report', desc: 'Retention, LTV & cohort analysis', lastGenerated: '15 Jul 2024', category: 'Customers' },
  { id: 6, icon: '🛒', title: 'Marketplace Performance', desc: 'Amazon, Flipkart & Myntra deep dive', lastGenerated: '10 Jul 2024', category: 'Sales' },
  { id: 7, icon: '📈', title: 'Financial P&L Report', desc: 'Revenue, costs, margins & profitability', lastGenerated: '5 Jul 2024', category: 'Finance' },
  { id: 8, icon: '🚚', title: 'Operations SLA Report', desc: 'Courier performance & delivery metrics', lastGenerated: '1 Jul 2024', category: 'Operations' },
];

const scheduledReports = [
  { name: 'Weekly Performance Summary', frequency: 'Every Monday', recipients: 'team@eternz.com', format: 'PDF', active: true },
  { name: 'Monthly P&L Report', frequency: '1st of month', recipients: 'cfo@eternz.com', format: 'Excel', active: true },
  { name: 'Daily Orders Digest', frequency: 'Daily 9 AM', recipients: 'ops@eternz.com', format: 'Email', active: false },
];

const recentReports = [
  { name: 'Monthly Performance — August 2024', type: 'Sales', generated: '1 Aug 2024', size: '2.4 MB' },
  { name: 'Campaign ROI — July 2024', type: 'Marketing', generated: '28 Jul 2024', size: '1.8 MB' },
  { name: 'Inventory Health — Q2 2024', type: 'Operations', generated: '20 Jul 2024', size: '3.1 MB' },
...
const METRICS = ['Gross Sales', 'Net Sales', 'Orders', 'AOV', 'ROAS', 'Contribution Margin', 'Return Rate', 'New Customers'];
    ...
    <AppLayout currentPath={pathname}>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Generate, schedule & download analytics reports</p>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {reportTemplates.map((report) => (
              <div key={report.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 hover:shadow-md transition-shadow flex flex-col">
                <div className="flex items-start justify-between mb-3">
                  <span className="text-3xl">{report.icon}</span>
                  <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${CATEGORY_COLORS[report.category]}`}>{report.category}</span>
                </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {/* Metrics */}
            <div>
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Select Metrics</p>
            ...
            {/* Grouping & Date */}
            <div>
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Grouping</p>
          ...
          {selectedMetrics.length > 0 && (
            <div className="mt-4 p-4 bg-gray-50 rounded-lg border border-dashed border-gray-200">
              <p className="text-xs text-gray-400 text-center">
                Preview: <span className="font-semibold text-gray-600">{selectedMetrics.join(', ')}</span> as <span className="font-semibold text-gray-600">{selectedChart}</span> grouped by <span className="font-semibold text-gray-600">{selectedGrouping}</span>
              </p>
            </div>
          )}
        </div>
                ...
                {recentReports.map((report, i) => (
                  <tr key={i} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-5 py-3">
                      <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${CATEGORY_COLORS[report.type]}`}>{report.type}</span>
                    </td>
