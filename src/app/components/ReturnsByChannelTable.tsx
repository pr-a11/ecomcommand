...
function ReturnPctBadge({ pct }: { pct: number }) {
  ...
  if (pct < 6) return <span className="text-xs font-600 text-amber-600 bg-amber-50 px-1.5 py-0.5 rounded">{pct}%</span>;
  return <span className="text-xs font-600 text-negative bg-red-50 px-1.5 py-0.5 rounded">{pct}%</span>;
}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border">
              <th className="text-left text-xs font-600 text-muted-foreground pb-2 pr-6">Channel</th>
              <th className="text-right text-xs font-600 text-muted-foreground pb-2 pr-6">Returned Orders</th>
              <th className="text-right text-xs font-600 text-muted-foreground pb-2">Return %</th>
            </tr>
          </thead>
          <tbody>
            {returnsByChannelData.map((row) => (
              <tr key={`ret-${row.channel}`} className="border-b border-border/50 hover:bg-muted/40 transition-colors">
