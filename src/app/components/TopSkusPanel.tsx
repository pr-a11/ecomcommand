              ...
              <div className="flex items-start gap-2 min-w-0">
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    ...
                    <span className={`text-xs font-500 px-1.5 py-0.5 rounded-full ${categoryColors[sku.category] || 'bg-muted text-muted-foreground'}`}>
                      {sku.category}
                    </span>
                  </div>
                  <p className="text-sm font-500 text-foreground mt-0.5 truncate">{sku.name}</p>
                  <p className="text-xs text-muted-foreground">{sku.units}u · {sku.orders}o</p>
                </div>
              </div>
