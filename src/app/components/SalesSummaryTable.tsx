            ...
            {salesSummaryData?.map((row) => (
              <tr
                ...
                className={`border-b border-border/50 hover:bg-muted/40 transition-colors ${
                  row?.isTotal
                    ? 'bg-muted/30 font-700'
                    : row?.isSubtotal
                      ? 'bg-muted/20 font-600'
                      : ''
                }`}
              >
                <td
                  className={`py-2.5 pr-4 text-sm ${row?.isTotal ? 'font-700 text-foreground' : row?.isSubtotal ? 'font-600' : 'text-foreground'}`}
                >
                  {row?.label}
                </td>
                <td
                  className={`py-2.5 pr-4 text-right tabular-nums text-sm ${row?.isTotal || row?.isSubtotal ? 'font-600' : 'text-muted-foreground'}`}
                >
                  {row?.orders}
                </td>
                <td
                  className={`py-2.5 text-right tabular-nums text-sm font-600 ${
                    row?.isTotal
                      ? 'text-primary text-base font-700'
                      : row?.isSubtotal
                        ? 'text-foreground'
                        : !row?.isPositive
                          ? 'text-negative'
