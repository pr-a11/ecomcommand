  ...
  return (
    <AppLayout currentPath={pathname}>
      <div className="space-y-6">
        ...
        {kpi && (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[kpi.revenue, kpi.orders, kpi.returns, kpi.rating].map((metric: any) => {
              ...
              const isPositive = isNegativeGood ? (metric.change ?? 0) <= 0 : (metric.change ?? 0) >= 0;
              const val = metric.label === 'Avg Rating' ? metric.value.toFixed(1) + '★'
                : metric.isPercent
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {activeTab === 'amazon' && (
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
              <h3 className="text-base font-semibold text-gray-900 mb-1">Best Seller Rank (Lower = Better)</h3>
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2">
                <div className="w-12 h-12 rounded-full flex items-center justify-center border-4" style={{ borderColor: color }}>
                  <span className="text-sm font-bold" style={{ color }}>{kpi?.listingHealth ?? 0}</span>
                </div>
              </div>
            </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
          <h3 className="text-base font-semibold text-gray-900 mb-4">Cross-Platform Revenue Comparison</h3>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50">
                  {['Product', 'SKU', 'Shopify', 'Amazon', 'Flipkart', 'Myntra', 'Total', 'Status'].map((h) => (
              ...
              <tbody className="divide-y divide-gray-50">
                {inventorySyncData?.map((row: any) => (
                  <tr key={row.sku} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-sm text-center font-bold text-gray-900">{row.total}</td>
