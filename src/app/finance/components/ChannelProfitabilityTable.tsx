...
function MarginBadge({ pct }: { pct: number }) {
  if (pct >= 65)
    return (
      <span className="text-xs font-600 text-primary bg-gray-100 px-2 py-0.5 rounded-full">
        {pct}%
      </span>
    );
  if (pct >= 55)
    return (
      <span className="text-xs font-600 text-amber-700 bg-amber-50 px-2 py-0.5 rounded-full">
        {pct}%
      </span>
    );
  return (
    <span className="text-xs font-600 text-negative bg-red-50 px-2 py-0.5 rounded-full">
      {pct}%
    </span>
  );
}
  ...
  return (
    <div className="chart-card">
      ...
      <p className="text-xs text-muted-foreground mb-4">
        Net margin % and realisation per channel · Last 30 days
      </p>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border">
              ...
              <th className="text-right text-xs font-600 text-muted-foreground pb-2">
                Net Margin %
              </th>
            </tr>
          </thead>
