...
import { Info, Star, ChevronRight, Download, RefreshCw, CheckCircle, AlertCircle } from 'lucide-react';

// ─── Mock Data ────────────────────────────────────────────────────────────────

const kpiData = [
  { label: 'TOTAL CUSTOMERS', value: '50', change: '+4.2%', sub: 'Growing customer base', sparkline: [30, 32, 35, 33, 38, 40, 42, 45, 44, 47, 49, 50] },
  { label: 'NEW CUSTOMERS', value: '45', change: '+2.3%', sub: 'Shopify D2C · first-time buyers', sparkline: [28, 30, 32, 29, 34, 36, 38, 40, 39, 42, 44, 45] },
  { label: 'RETURNING CUSTOMERS', value: '5', change: '+25.0%', sub: 'Shopify D2C · repeat buyers', sparkline: [2, 2, 3, 3, 3, 4, 4, 4, 5, 5, 5, 5] },
  { label: 'REPEAT RATE', value: '10.0%', change: '+20.5%', sub: 'Below benchmark — focus retention', sparkline: [5, 5, 6, 6, 7, 7, 8, 8, 9, 9, 10, 10] },
  { label: 'AVG LTV', value: '₹6,043', change: '+5.7%', sub: 'Across 48 customers', sparkline: [4200, 4400, 4600, 4800, 5000, 5100, 5300, 5500, 5700, 5800, 5950, 6043] },
  { label: 'STORE RATING', value: '★ 4.8', change: '', sub: '5 reviews · Excellent', sparkline: [4.2, 4.3, 4.5, 4.5, 4.6, 4.7, 4.7, 4.8, 4.8, 4.8, 4.8, 4.8] },
];
...
const retentionStats = [
  ...
  { label: '2ND ORDER ≤ 90 DAYS', value: '1.8%', sub: 'bought again within 3 months', highlight: false },
  { label: 'AVG DAYS TO 2ND ORDER', value: '9 days', sub: 'for returning customers', highlight: false },
];
...
const reviewsData = [
  {
    id: 1, stars: 5, verified: true, date: '14 Aug',
  ...
  {
    id: 4, stars: 5, verified: true, date: '05 Aug',
  ...
  {
    id: 5, stars: 5, verified: true, date: '01 Aug',
...
function StarRow({ count }: { count: number }) {
  return (
    <span className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((s) => (
        <Star key={s} size={12} className={s <= count ? 'text-amber-400 fill-amber-400' : 'text-gray-200 fill-gray-200'} />
      ))}
    </span>
  );
}
    ...
    <AppLayout currentPath={pathname}>
      <div className="space-y-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Customer analytics, retention &amp; reputation</p>
        </div>

        {/* ── KPI Row ─────────────────────────────────────────────────────── */}
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
          {kpiData.map((kpi) => (
            <div key={kpi.label} className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 flex flex-col justify-between min-h-[120px]">
              <div className="flex items-center gap-1 mb-1">
                <p className="text-[10px] font-semibold text-gray-400 tracking-wide uppercase leading-tight">{kpi.label}</p>
                <Info size={10} className="text-gray-300 flex-shrink-0" />
              </div>
              <div className="flex items-baseline gap-2">
                <p className="text-xl font-bold text-gray-900">{kpi.value}</p>
                {kpi.change && (
                  <span className="text-[10px] font-semibold text-gray-700 bg-gray-100 px-1.5 py-0.5 rounded-full">{kpi.change}</span>
                )}
              </div>
        ...
        <div>
          <div className="flex items-center gap-2 mb-3">
            <span className="text-[11px] font-semibold text-gray-400 uppercase tracking-widest">Customer Identity — Shopify D2C</span>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
              <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">Shopify D2C · Highest spend this period</p>
                </div>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-gray-100">
                      <th className="text-left text-xs font-semibold text-gray-400 px-5 py-3 w-8">#</th>
                      <th className="text-left text-xs font-semibold text-gray-400 px-3 py-3">CUSTOMER</th>
                      <th className="text-left text-xs font-semibold text-gray-400 px-3 py-3">LOCATION</th>
                    </tr>
                  </thead>
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
              <div className="flex items-start justify-between mb-1">
                <div>
                  <div className="flex items-center gap-2">
                    <h3 className="text-sm font-bold text-gray-900">Customer Retention &amp; Repeat Purchase</h3>
                    <Info size={13} className="text-gray-400" />
                  </div>
                  <p className="text-xs text-gray-400 mt-1 leading-relaxed">
                    Repeat purchase is tracked for Shopify D2C only — marketplaces anonymise buyers. Each row is customers who first bought that month; each cell is the % who bought again that many months later (darker = stickier).
                  </p>
                </div>
                <div className="flex items-center gap-2 flex-shrink-0 ml-3">
                  <button className="text-gray-400 hover:text-gray-600"><RefreshCw size={13} /></button>
                  <button className="flex items-center gap-1 text-xs text-gray-500 hover:text-gray-800"><Download size={12} /> CSV</button>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 mt-4">
                {retentionStats.map((s) => (
                  <div key={s.label} className={`rounded-lg p-3 ${s.highlight ? 'bg-gray-50 border border-gray-200' : 'bg-gray-50'}`}>
                    <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wide">{s.label}</p>
                    <p className={`text-lg font-bold mt-0.5 ${s.highlight ? 'text-gray-900' : 'text-gray-900'}`}>{s.value}</p>
              ...
              {/* Cohort Table */}
              <div className="mt-4">
                <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest mb-2">Repeat Purchase — Shopify D2C</p>
                    ...
                    {cohortData.map((row) => (
                      <tr key={row.cohort}>
                        ...
                        <td className="py-2 px-3 text-center">
                          <span className={`inline-block px-3 py-1 rounded font-bold ${row.m0Highlight ? 'bg-gray-900 text-white' : 'text-gray-700'}`}>
                            {row.m0}
                          </span>
                        </td>
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11px] font-semibold text-gray-400 uppercase tracking-widest">Reviews &amp; Reputation</span>
            </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 mb-5">
              ...
              <div className="flex flex-col items-center flex-shrink-0">
                <div className="relative w-28 h-28">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={ratingDonutData} cx="50%" cy="50%" innerRadius={38} outerRadius={52} dataKey="value" startAngle={90} endAngle={-270} paddingAngle={2}>
                        {ratingDonutData.map((entry, i) => <Cell key={i} fill={entry.color} />)}
                      </Pie>
                    </PieChart>
                  </ResponsiveContainer>
                  ...
                  <div key={row.stars} className="flex items-center gap-2">
                    ...
                    <div className="flex-1 bg-gray-100 rounded-full h-2 overflow-hidden">
                      <div
                        ...
                        style={{ width: `${row.pct}%`, backgroundColor: row.stars >= 4 ? '#111827' : row.stars === 3 ? '#d97706' : '#dc2626' }}
                      />
                    </div>
            ...
            <div className="mt-4 flex items-center gap-2 bg-amber-50 border border-amber-100 rounded-lg px-4 py-2.5">
              ...
              <p className="text-xs text-amber-700 font-medium">No negative reviews in this period</p>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              {reviewsData.map((review) => (
                <div key={review.id} className="border border-gray-100 rounded-xl p-4 hover:shadow-sm transition-shadow">
                  ...
                  <p className="text-sm font-semibold text-gray-900 mb-1 leading-snug">{review.title}</p>
