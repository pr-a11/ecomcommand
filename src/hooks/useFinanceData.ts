  ...
  useEffect(() => {
    ...
    async function fetchAll() {
      ...
      try {
        ...
        const [kpiRes, waterfallRes, netSalesRes, marginRes, geoRes, profitRes, feesRes] =
          await Promise.all([
            supabase
              .from('finance_kpis')
              .select('*')
            ...
            supabase
              .from('net_sales_over_time')
              .select('*')
              .eq('user_id', user.id)
              .order('sort_order', { ascending: true }),
            supabase
              .from('contribution_margin_trend')
              .select('*')
              .eq('user_id', user.id)
              .order('sort_order', { ascending: true }),
            supabase
        ...
        if (waterfallRes.data && waterfallRes.data.length > 0) {
          setPlWaterfallData(
            waterfallRes.data.map((r: any) => ({
              label: r.label,
              value: r.value,
              type: r.entry_type as 'positive' | 'negative' | 'subtotal' | 'total',
            }))
          );
        }

        if (netSalesRes.data && netSalesRes.data.length > 0) {
          setNetSalesOverTimeData(
            netSalesRes.data.map((r: any) => ({
              date: r.date_label,
              current: r.current_period,
              previous: r.previous_period,
            }))
          );
        }

        if (marginRes.data && marginRes.data.length > 0) {
          setContributionMarginTrendData(
        ...
        if (profitRes.data && profitRes.data.length > 0) {
          setChannelProfitabilityData(
            profitRes.data.map((r: any) => ({
              id: r.id,
              channel: r.channel,
              grossSales: r.gross_sales,
              fees: r.fees,
              netRealisation: r.net_realisation,
              netMarginPct: r.net_margin_pct,
              takeRate: r.take_rate,
            }))
          );
        }

        if (feesRes.data && feesRes.data.length > 0) {
          setMarketplaceFeeData(
            feesRes.data.map((r: any) => ({
              id: r.id,
              marketplace: r.marketplace,
              referralFee: r.referral_fee,
              closingFee: r.closing_fee,
    ...
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'channel_profitability' },
        fetchAll
      )
      .subscribe();
