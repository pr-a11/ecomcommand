      ...
      try {
        ...
        if (kpiRes.data) {
          ...
          setOperationsKpis({
            ...
            deliveredToday: {
              value: k.delivered_today,
              label: 'Delivered Today',
              change: k.delivered_today_change,
            },
            returnRequests: {
              value: k.return_requests,
              label: 'Return Requests',
              change: k.return_requests_change,
            },
            avgDeliveryDays: {
              value: k.avg_delivery_days,
              label: 'Avg Delivery Time',
              sub: 'days',
              change: k.avg_delivery_days_change,
            },
          });
          ...
          setOrderFunnelData([
            ...
            {
              stage: 'Confirmed',
              count: Math.round((k.dispatched + k.pending) * 0.979),
              pct: 97.9,
            },
            { stage: 'Packed', count: Math.round((k.dispatched + k.pending) * 0.958), pct: 95.8 },
            {
              stage: 'Dispatched',
              count: k.dispatched,
              pct: Math.round((k.dispatched / (k.dispatched + k.pending)) * 100 * 10) / 10,
            },
            {
              stage: 'In Transit',
              count: k.in_transit,
              pct: Math.round((k.in_transit / (k.dispatched + k.pending)) * 100 * 10) / 10,
            },
            {
              stage: 'Delivered',
              count: k.delivered_today,
              pct: Math.round((k.delivered_today / (k.dispatched + k.pending)) * 100 * 10) / 10,
            },
          ]);
        }
        ...
        if (courierRes.data && courierRes.data.length > 0) {
          setCourierData(
            courierRes.data.map((r: any) => ({
              courier: r.courier,
              shipments: r.shipments,
              deliveredPct: r.delivered_pct,
              avgDays: r.avg_days,
              ndrRate: r.ndr_rate,
              costPerShipment: r.cost_per_shipment,
            }))
          );
        }
