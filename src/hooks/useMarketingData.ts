...
export function useMarketingData() {
  ...
  const [conversionFunnelData, setConversionFunnelData] = useState<any[]>(
    mockMarketing.conversionFunnelData
  );
  const [channelSessionsData, setChannelSessionsData] = useState<any[]>(
    mockMarketing.channelSessionsData
  );
    ...
    async function fetchAll() {
      ...
      try {
        ...
        if (kpiRes.data) {
          ...
          setMarketingKpiData({
            grossSales: {
              ...
              label: 'Gross Sales',
              caption: 'Shopify + Marketplace',
            },
            totalAdSpend: {
              value: k.total_ad_spend,
              change: k.total_ad_spend_change,
              label: 'Total Ad Spend',
              caption: 'Sharp spend cut — check campaigns',
              alert: true,
            },
            blendedROAS: {
              value: k.blended_roas,
              change: 0,
              label: 'Blended ROAS',
              caption: 'Strong ROAS',
            },
            attributedSales: {
              value: k.attributed_sales,
              change: 0,
              label: 'Attributed Sales',
              caption: '21% of Gross · reported',
            },
            attributedROAS: {
            ...
            adSpendPct: {
              value: k.ad_spend_pct,
              change: 0,
              label: 'Ad Spend % of Rev.',
              caption: 'of Gross Sales',
              isPercent: true,
            },
            cac: {
              value: k.cac,
              change: 0,
              label: 'CAC',
              caption: 'per new customer · high CAC',
              alert: true,
            },
          });
        }
        ...
        if (campRes.data && campRes.data.length > 0) {
          setCampaignData(
            campRes.data.map((r: any) => ({
              id: r.id,
              platform: r.platform,
              name: r.name,
              spend: r.spend,
              attributedSales: r.attributed_sales,
              orders: r.orders,
              roas: r.roas,
        ...
        if (sessionsRes.data && sessionsRes.data.length > 0) {
          setChannelSessionsData(
            sessionsRes.data.map((r: any) => ({
              id: r.id,
              channel: r.channel,
              sessions: r.sessions,
              orders: r.orders,
              sales: r.sales,
              convRate: r.conv_rate,
            }))
          );
        }
        // else keep mock data
